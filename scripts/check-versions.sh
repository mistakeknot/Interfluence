...
if [ -f "$MARKETPLACE" ]; then
    MARKETPLACE_VERSION=$(grep -A10 '"interfluence"' "$MARKETPLACE" | grep '"version"' | sed 's/.*"\([0-9][^"]*\)".*/\1/')
...
# Check server/package.json if it exists
if [ -f "$REPO_ROOT/server/package.json" ]; then
    SERVER_VERSION=$(grep -E '"version"' "$REPO_ROOT/server/package.json" | head -1 | sed 's/.*"\([0-9][^"]*\)".*/\1/')
    if [ -n "$SERVER_VERSION" ] && [ "$SERVER_VERSION" != "$PLUGIN_VERSION" ]; then
        echo -e "${RED}server/package.json version drift!${NC}" >&2
        echo "" >&2
        echo "  .claude-plugin/plugin.json:  $PLUGIN_VERSION" >&2
        echo "  server/package.json:         $SERVER_VERSION" >&2
        echo "" >&2
        echo "Run: scripts/bump-version.sh $PLUGIN_VERSION" >&2
        exit 1
    fi
