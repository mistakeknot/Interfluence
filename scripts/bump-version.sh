...
if [ -f "$REPO_ROOT/server/package.json" ]; then
    SERVER_CURRENT=$(grep -E '"version"' "$REPO_ROOT/server/package.json" | head -1 | sed 's/.*"\([0-9][^"]*\)".*/\1/')
    if [ -n "$SERVER_CURRENT" ]; then
        update_file \
            "$REPO_ROOT/server/package.json" \
            "\"version\": \"$SERVER_CURRENT\"" \
            "\"version\": \"$VERSION\"" \
            "server/package.json"
    fi
fi
...
MARKETPLACE_CURRENT=$(grep -A10 '"interfluence"' "$MARKETPLACE_ROOT/.claude-plugin/marketplace.json" | grep '"version"' | sed 's/.*"\([0-9][^"]*\)".*/\1/')
update_file \
    "$MARKETPLACE_ROOT/.claude-plugin/marketplace.json" \
    "\"version\": \"$MARKETPLACE_CURRENT\"" \
    "\"version\": \"$VERSION\"" \
    "interagency-marketplace/marketplace.json (interfluence entry)"
...
echo ""
cd "$REPO_ROOT"
git add .claude-plugin/plugin.json server/package.json
git commit -m "chore: bump version to $VERSION"
git push
echo -e "${GREEN}Pushed interfluence${NC}"

cd "$MARKETPLACE_ROOT"
git add .claude-plugin/marketplace.json
git commit -m "chore: bump interfluence to v$VERSION"
git push
echo -e "${GREEN}Pushed interagency-marketplace${NC}"

echo ""
echo -e "${GREEN}Done!${NC} interfluence v$VERSION"
echo ""
